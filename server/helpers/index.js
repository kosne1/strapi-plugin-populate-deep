...
const populateKey = (populate, key, value, maxDepth, ignore) => {
  ...
  case "component":
    ...
    break;
  case "dynamiczone":
    const dynamicPopulate = getDynamicPopulate(value.components, maxDepth, ignore);
...
const getDynamicPopulate = (components, maxDepth, ignore) => {
  /**
...
const getFullPopulateObject = (
  ...
  if (ignore && !ignore.includes(modelUid))
    ignore.push(modelUid);

  for (const [key, value] of Object.entries(
    getModelPopulationAttributes(model)
  )) {
    if (value?.target && ignore?.includes(value?.target)) continue;
